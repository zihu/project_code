#!/bin/bash 
# this script tries to merge traces for each 15 minutes time bin. 
...
if [ -e "$STATUS_FN" ]; then
  ...
  echo $CMP_EST $CMP_EET $CMP_SST $CMP_SET
  ...
  if [ $CMP_SST -eq 1 ] && [ $CMP_EET -eq 1 ];then
    ...
    /bin/cp $INPUTFILE $BIN_TMP_DIR
  elif [ $CMP_SST -eq 0 ] && [ $CMP_EET -eq 1 ] && [ $CMP_EST -eq 1 ]; then
    echo "second half"
  ...
  elif [ $CMP_SST -eq 1 ] && [ $CMP_EET -eq 0 ] && [ $CMP_SET -eq 1 ]; then
    echo "firt half"
    ...
    # delete traces of the current bin
    /bin/rm -rf $BIN_TMP_DIR/*
    # start the new bin and mv current trace into this bin
    /bin/cp $INPUTFILE $BIN_TMP_DIR
    # update the status file
    BIN_ST=$TRACE_ST
    BIN_ET=`echo "$BIN_ST + $INTERVAL" | bc`
    echo -e "$BIN_ST\t$BIN_ET\t$TRACE_SEQN" > $STATUS_FN
  elif [ $CMP_EST -eq 0 ]; then
    echo "old trace, ignore"
    echo $basename >> $TMPERR 
  fi
else
  ...
  # mv current trace into this bin
  /bin/cp $INPUTFILE $BIN_TMP_DIR
fi
